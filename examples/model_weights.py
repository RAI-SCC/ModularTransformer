...
from examples.util import load_model_weights, Dataset, LossFunction
...
def plot_weights(
    ...
    plt.ylabel("output")
    # plt.title(name)
    plt.savefig(f"examples/plots/model-weights/{name}-{dataset}-{input_length}-{output_length}-{loss_function}.pdf")
    plt.show()
...
if __name__ == '__main__':
    ...
    plot_weights(weights_q_1, "quadratic-layer-1", dataset, input_length, output_length, loss_function)
    weights_q_2 = quadratic_model.lin1.weight.detach().numpy().__abs__()
    plot_weights(weights_q_2, "quadratic-layer-2", dataset, input_length, output_length, loss_function)
    ...
    linear_model = get_linear_model(input_size=input_length, output_size=output_length, hidden_size=_hidden_size)
